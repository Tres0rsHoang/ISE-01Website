...
class DashboardStudent extends Component {
    ...
    componentDidMount() {
        ...
        axios.get('/Student', config).then(result => {
            console.log(result);
            this.setState({
    ...
    render() {
        if (localStorage.getItem("accessToken") === '' || this.state.user.usertype !== 3) return <EmptyPage/>;
        const res = []
        this.state.courseList.forEach((course, index) => {
            var courseURL = "/Student/" + course.courseid;
            res.push(
