...
function LectureManagement(){
    ...
    useEffect(() => {
        ...
        axios.get(`/Admin/LectureManagement${window.location.search}`, config).then(
            result=>{
                setState({
    ...
    const DeleteID = (event) =>{
        ...
        const conf = window.confirm("Do you really want to delete this Lecture. ID: " + ID);
        if (conf){
            axios.post('/Admin/DeleteLecture/'+ID, config).then(res=>{})
    ...
    for (let i = 0;i<state.List.length;i++){
        ...
        res.push(
            <tr>
                ...
                <td>{dateFormat}</td>
                <td><a href={"/Admin/EditLecture/"+state.List[i].id}><Button><FaPen/></Button></a></td>
        ...
        <Fragment>
            <AdminNavBar>
                <div style={{width: "100%", marginTop: "20px"}}>
                    <div className={"fs-4 fw-bold"} style={{display: "inline-block", marginLeft: "50px"}}>Lecture List
                    </div>
                    <Link to={'/Admin/AddNewLecture'}>
                        <button style={{
